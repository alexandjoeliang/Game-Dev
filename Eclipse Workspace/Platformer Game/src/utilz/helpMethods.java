...
public class helpMethods {
	...
	public static boolean CanMoveHere(float x, float y, float width, float height, int[][] lvlData, int dir) {
		...
		//if(dir == Directions.LEFT){
			for(int yStep = 0; yStep < (int) (height); yStep += gameClass.SCALE * gameClass.PLAYER_TO_TILE_RATIO) {
		...
		//if(dir == Directions.UP){
			for(int xStep = 0; xStep < (int) (width); xStep += gameClass.SCALE * gameClass.PLAYER_TO_TILE_RATIO) {
